...
{
	UE_LOG(LogServerWebSocket, Log, TEXT("client: %s"), ANSI_TO_TCHAR(line));
}
...
:IsServerSide(false)
{
	...
	if (SockFd == -1)
	{
		UE_LOG(LogServerWebSocket, Error, TEXT("Socket creation failed "));
	}
	else
	{
		UE_LOG(LogServerWebSocket, Warning, TEXT(" Socked %d created "), SockFd);
	}
	...
	{
		UE_LOG(LogServerWebSocket, Warning, TEXT("WSAStringToAddress failed "));
		return;
	}
#else
	if (inet_pton(AF_INET, TCHAR_TO_ANSI(*ServerAddress.ToString(false)), &RemoteAddr.sin_addr) != 1)
	{
		UE_LOG(LogServerWebSocket, Warning, TEXT("inet_pton failed to %s"), *ServerAddress.ToString(false));
		return;
	}
#endif
	...
	UE_LOG(LogServerWebSocket, Warning, TEXT(" Connect socket returned %d to %s. Error Code: %d"), Ret, *ServerAddress.ToString(false), ((Ret != 0) ? errno : 0));
#endif
}

#if USE_LIBWEBSOCKET
FNetWebSocket::FNetWebSocket(WebSocketInternalContext* InContext, WebSocketInternal* InWsi)
	: Context(InContext)
	, Wsi(InWsi)
	, Protocols(nullptr)
	, IsServerSide(true)
...
bool FNetWebSocket::Send(const uint8* Data, uint32 Size, bool bPrependSize)
{
	TArray<uint8> Buffer;
...
void FNetWebSocket::SetReceiveCallBack(FNetWebSocketPacketReceivedCallBack CallBack)
{
	ReceivedCallback = CallBack;
}

void FNetWebSocket::SetSocketClosedCallBack(FNetWebSocketInfoCallBack CallBack)
{
	SocketClosedCallback = CallBack;
}

FString FNetWebSocket::RemoteEndPoint(bool bAppendPort)
{
...
FString FNetWebSocket::LocalEndPoint(bool bAppendPort)
{
#if USE_LIBWEBSOCKET
	int sock = lws_get_socket_fd(Wsi);
...
void FNetWebSocket::Tick()
{
	HandlePacket();
}

void FNetWebSocket::HandlePacket()
{
#if USE_LIBWEBSOCKET
	...
	if (Res == -1) {
		UE_LOG(LogServerWebSocket, Warning, TEXT("Select Failed!"));
		return;
	}
...
void FNetWebSocket::Flush()
{
	...
	while (OutgoingBuffer.Num() > 0 && !IsServerSide)
	{
		...
		{
			UE_LOG(LogServerWebSocket, Warning, TEXT("Unable to flush all of OutgoingBuffer in FNetWebSocket."));
			break;
		}
	};
}

TArray<uint8> FNetWebSocket::GetRawRemoteAddr(int32& OutPort)
{
...
void FNetWebSocket::SetConnectedCallBack(FNetWebSocketInfoCallBack CallBack)
{
	ConnectedCallBack = CallBack;
}

void FNetWebSocket::SetErrorCallBack(FNetWebSocketInfoCallBack CallBack)
{
	ErrorCallBack = CallBack;
}

void FNetWebSocket::OnReceive(void* Data, uint32 Size)
{
#if USE_LIBWEBSOCKET
...
void FNetWebSocket::OnRawRecieve(void* Data, uint32 Size)
{
#if USE_LIBWEBSOCKET
...
void FNetWebSocket::OnRawWebSocketWritable(WebSocketInternal* wsi)
{
	...
	while (DataToSend)
	{
		...
		{
			UE_LOG(LogServerWebSocket, Warning, TEXT("Could not write all '%d' bytes to socket"), DataToSend);
		}
		DataToSend-=Sent;
	}
	...
	while (DataToSend)
	{
		...
		if (Result == -1)
		{
			...
			UE_LOG(LogServerWebSocket, Error, TEXT("Could not write %d bytes"), Packet.Num());
			ErrorCallBack.ExecuteIfBound();
			return;
		}
		UE_CLOG((uint32)Result < DataToSend, LogServerWebSocket, Warning, TEXT("Could not write all '%d' bytes to socket"), DataToSend);
		DataToSend-=Result;
	}
		...
		size_t Len)
{
	...
	FNetWebSocket* Socket = (FNetWebSocket*)lws_context_user(Context);
	switch (Reason)
	{
