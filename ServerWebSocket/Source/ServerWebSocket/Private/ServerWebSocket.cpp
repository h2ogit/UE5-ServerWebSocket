...
static int unreal_networking_server(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len);
	...
	{
		UE_LOG(LogServerWebSocket, Log, TEXT("websocket server: %s"), ANSI_TO_TCHAR(line));
	}
#endif // UE_BUILD_SHIPPING

#endif // USE_LIBWEBSOCKET

bool FServerWebSocket::IsHttpEnabled() const
{
	return bEnableHttp;
}

void FServerWebSocket::EnableHTTPServer(TArray<FNetWebSocketHttpMount> InDirectoriesToServe)
{
#if USE_LIBWEBSOCKET
	bEnableHttp = true;
	...
	for(int i = 0; i < NMounts; i++) {
		...
		FNetWebSocketHttpMount& MountDir = DirectoriesToServe[i];
...
bool FServerWebSocket::Init(uint32 Port, FNetWebSocketClientConnectedCallBack CallBack)
{
#if USE_LIBWEBSOCKET
#if !UE_BUILD_SHIPPING
	// setup log level.
...
FServerWebSocket::~FServerWebSocket()
{
#if USE_LIBWEBSOCKET
	...
	)
{
	...
	FServerWebSocket* Server = (FServerWebSocket*)lws_context_user(Context);

	switch (Reason)
	{
		case LWS_CALLBACK_ESTABLISHED:
			{
				BufferInfo->Socket = new FNetWebSocket(Context, Wsi);
